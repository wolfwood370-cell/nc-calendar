...
DECLARE
  ...
  v_grace int;
BEGIN
  -- Bypass auth check for backend roles (pg_cron auto-renew job, service_role
  -- maintenance, postgres migrations). These callers have no auth.uid() and
  -- legitimately need to align blocks across all clients.
