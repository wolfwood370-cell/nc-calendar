...
BEGIN
  ...
  LOOP
    ...
    IF v_blk.start_date <> v_new_start OR v_blk.end_date <> v_new_end THEN
      UPDATE public.training_blocks tb SET start_date = v_new_start, end_date = v_new_end WHERE tb.id = v_blk.id;
      UPDATE public.block_allocations ba
      SET valid_until = (v_new_end + v_grace * INTERVAL '1 day')::date
      WHERE ba.block_id = v_blk.id
        AND ba.valid_until IS NOT NULL
        AND ba.valid_until = (v_blk.end_date + v_grace * INTERVAL '1 day')::date;
