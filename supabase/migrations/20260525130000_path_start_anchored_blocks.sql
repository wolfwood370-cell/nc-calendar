...
-- ==========================================================================
-- Closes the discrepancy between two date sources for a client's path:
--
--   * Frontend Timeline UI (trainer.clients.$id.tsx:273-307) computes
--     week dates as `addDays(profiles.path_start_date, i*7)` — totally
--     ignoring training_blocks.start_date.
--
--   * Backend logic (ensure_client_block_state, validate_booking_block_
--     allocation, the trainer card counter) reads training_blocks.
--     start_date / end_date directly.
--
-- When the two diverge — e.g. Simone Sambataro shows in the UI as starting
-- 20/04/2026 but his Blocco 1 in DB starts 12/05/2026 — counters become
-- inconsistent, auto-renewal triggers at the wrong moment, and the
-- "Blocco N+1" never materializes when expected.
--
-- This migration anchors training_blocks to profiles.path_start_date and
-- ships:
--
--   1. audit_misaligned_blocks(p_coach_id): read-only diagnostic listing
--      every recurring client whose first block doesn't start on
--      path_start_date OR whose block chain isn't contiguous. Run first
--      to understand the scope of repair.
--
--   2. repair_blocks_alignment(p_client_id): per-client repair that
--      rewrites every block's start_date/end_date cumulatively from
--      path_start_date (respecting each block's own duration_days). Also
--      adjusts block_allocations.valid_until accordingly. Idempotent.
--
--   3. ensure_client_block_state v2: now calls repair_blocks_alignment
--      internally before checking expiry, so the next render of any
--      dashboard auto-corrects the data without manual intervention.
--
-- Safety: bookings keep their `block_id` reference; their scheduled_at
-- is never modified. After repair, bookings naturally fall into the
-- correctly-dated blocks (their date stays put, the block dates around
-- them shift to match path_start_date). The FIFO trigger
-- validate_booking_block_allocation reads valid_until from
-- block_allocations so consumption stays correct.
--
-- Clients without `profiles.path_start_date` (legacy onboarding before
-- the column existed) are skipped — repair returns 'no_anchor' and no
-- data is changed.
-- ==========================================================================

-- --------------------------------------------------------------------------
-- 1. audit_misaligned_blocks — diagnostic
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 2. repair_blocks_alignment — idempotent per-client fix
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  -- Loop through every non-deleted block ordered by sequence. Recompute
  -- start/end cumulatively from path_start_date, respecting each block's
  -- own duration_days (allows mixed-duration paths in the future).
  FOR v_blk IN
    SELECT
  ...
  LOOP
    ...
    IF v_prev_end IS NULL THEN
      -- First block anchors to path_start_date directly.
      v_new_start := v_path_start;
    ELSE
      -- Subsequent blocks chain after the previous one's end.
    ...
    IF v_blk.start_date <> v_new_start OR v_blk.end_date <> v_new_end THEN
      ...
      -- Allocations' valid_until tracks block end + grace. Keep them in
      -- sync so the FIFO booking trigger gates consumption correctly.
...
-- --------------------------------------------------------------------------
-- 3. ensure_client_block_state v2 — calls repair internally
-- --------------------------------------------------------------------------
...
BEGIN
  ...
  -- v2: auto-repair before any read. Idempotent — no rows changed if
  -- already aligned. Bypasses the trailing return of repair_blocks_
  -- alignment by reading its result into a discard variable, since
  -- plpgsql can't ignore a RETURN QUERY directly.
  PERFORM 1 FROM public.repair_blocks_alignment(p_client_id);

  -- Most recent non-deleted block (now guaranteed aligned to
  -- path_start_date if the profile has one).
  SELECT * INTO v_last
  FROM public.training_blocks
  ...
  -- Past end_date: close + maybe create successor.
  ...
  -- Auto-renew on: ensure successor exists.
...
COMMENT ON FUNCTION public.ensure_client_block_state(uuid) IS
  'v2 (20260525130000): anchors training_blocks to profiles.path_start_date '
  'via auto-call to repair_blocks_alignment before checking expiry. Single '
  'source of truth = path_start_date so Timeline UI + DB stay consistent.';
COMMENT ON FUNCTION public.repair_blocks_alignment(uuid) IS
  'Idempotent: recomputes start/end of every non-deleted training_block '
  'for the client cumulatively from profiles.path_start_date. Adjusts '
  'block_allocations.valid_until accordingly. No-op when path_start_date '
  'is NULL (legacy onboarding).';
COMMENT ON FUNCTION public.audit_misaligned_blocks(uuid) IS
  'Read-only audit: lists recurring clients whose first block date '
  'doesn''t match path_start_date OR whose block chain is non-contiguous. '
  'Used as diagnostic before repair_blocks_alignment.';
