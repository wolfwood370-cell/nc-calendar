...
export function CalendarGcalReview({ coachId, bookings, clientsMap, eventTypesMap }: Props) {
  ...
  function bookingLabel(b: BookingRow): string {
    ...
    const base =
      b.title ?? et?.name ?? (b.session_type ? sessionLabel(b.session_type) : "Sessione");
    ...
    <div className="mb-4 rounded-2xl border border-surface-container bg-white shadow-soft-blue overflow-hidden">
      ...
      >
        <span className="flex items-center gap-2 font-medium text-sm">
          ...
          {errored && (
            <span className="ml-1 rounded-full bg-danger-soft px-2 py-0.5 text-xs font-semibold text-danger-text">
      ...
      {open && (
        <div className="border-t border-surface-container px-4 py-3 space-y-4 text-sm">
          ...
          {platformOnly.length > 0 && (
            <div>
              <h4 className="flex items-center gap-2 font-semibold text-warning-text mb-2">
              ...
              <ul className="space-y-1">
                {platformOnly.map((b) => (
                  <li
                    key={b.id}
                    className="flex items-center justify-between gap-3 rounded-lg bg-warning-soft/60 px-3 py-2"
                  >
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <Select value={eventTypeId} onValueChange={setEventTypeId}>
                    <SelectTrigger>
                      <SelectValue placeholder="Predefinito (Sessione PT)" />
                    </SelectTrigger>
                    <SelectContent>
                      {eventTypes.map((et) => (
