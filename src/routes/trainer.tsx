...
import { AssignEventDialog } from "@/components/assign-event-dialog";
...
// open the shared «Assegna evento» dialog by navigating with
// `?reviewEventId=` (the dialog reads this directly from the route search).
...
function TrainerLayout() {
  ...
  // so the dialog can be reopened from anywhere by adding it back. replace:
  // «Indietro» del browser non riapre un evento appena assegnato.
  ...
  return (
    ...
    <div className="min-h-screen flex w-full bg-background md:bg-[linear-gradient(135deg,#f2f3f8,#e7e8ec)]">
      ...
      <div className="relative flex w-full flex-1 flex-col bg-background md:bg-transparent">
        ...
        {/* «Assegna evento» (passata 02) — reachable from any /trainer/* page
            via navigate({ search: { reviewEventId: bookingId } }). One
